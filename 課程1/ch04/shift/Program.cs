...
namespace shift
{
    class Program
    {
        ...
        {
            string str1 = "";
            int[] ary = new int[] { 111, 222, 333, 444, 555, 666 }; //宣告建立和設定陣列初值
            // 原始陣列
            Console.Write(" 原始陣列 : ");
            ...
            // 陣列右移
            Console.Write(" 陣列右移 : ");
            temp = ary[ary.Length - 1];         	// 將最後一個陣列元素複製到temp變數
            for (int i = ary.Length - 1; i >= 1; i--)
                ary[i] = ary[i - 1];         		// 將前面的陣列元素往後移動一個陣列元素
